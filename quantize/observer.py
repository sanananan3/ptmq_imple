...
# Observer : nn의 파라미터 값에 대한 통계를 수집한다.
# 양자화 시 필요한 최소/최대 값 , scale, zero point 등을 계산한다. 


def _transform_to_ch_axis(x, ch_axis): # ch_axis (채널 축)을 기준으로 입력 텐서 x를 변환
    if ch_axis == -1: # 채널 축이 없는 경우 x 그대로 반환
        return x
    else:
        x_dim = x.size()
        new_axis_list = [i for i in range(len(x_dim))] # 텐서의 차원 정보를 기반으로 새로운 축 리스트를 생성
        new_axis_list[ch_axis] = 0  # 채널 축을 0으로 재배치 
        new_axis_list[0] = ch_axis
        x_channel = x.permute(new_axis_list) # 채널 축을 이동시키기 위해 텐서 재배치
        y = torch.flatten(x_channel, start_dim=1) # 첫번째 채널부터 나머지 차원을 평평하게 만들어준다. 
        return y


class ObserverBase(nn.Module): # 모든 양자화 observer의 기본 클래스 => 양자화 시 필요한 통계를 수집하는 기능
    ...
    def __init__(self, bit=8, symmetric=False, ch_axis=-1):
        ...
        if self.symmetric: # 대칭일 때 양자화 값의 최소 최대 범위
            self.quant_min = -2 ** (self.bit - 1)
            self.quant_max = 2 ** (self.bit - 1) - 1
        else: # 비대칭일 때 양자화 값의 최소 최대 범위
            self.quant_min = 0
    ...
    def set_name(self, name): # 관찰자 이름 설정 / 모델의 다양한 레이어에서 이름으로 식별 가능 
        self.name = name

    @torch.jit.export
    # quantizaiton 파라미터인 scale, zero point 계산 
...
class MinMaxObserver(ObserverBase):
    '''
    calibration 데이터 셋의 최소/최대값 추적하여 양자화를 위한 범위를 설정 
    '''
    ...
    def forward(self, x_orig):
        ...
        if self.ch_axis == -1:
            min_val_cur, max_val_cur = torch._aminmax(x) # 텐서의 최소값과 최대값을 동시에 반환
        else:
            y = _transform_to_ch_axis(x, self.ch_axis)
            min_val_cur, max_val_cur = torch._aminmax(y, 1) # 텐서의 최소값과 최대값을 동시에 반환
...
class AvgMinMaxObserver(ObserverBase):
    """
    Average min/max calibration => 최소 최대값의 평균을 구하였기 떄문에 양자화 범위를 더 부드럽게 설정 가능
    """
...
class MSEObserver(ObserverBase):
    ...
    def __init__(self, bit=8, symmetric=False, ch_axis=-1):
        ...
        self.p = 2.4 # loss function에서 사용하는 p값 => L2 norm loss에 가까움
        self.num = 100  # candidate num / num 만큼의 후보를 탐색하며 최적화한다. 
        self.one_side_dist = None  # 'pos', 'neg', 'no' 입력값의 분포가 한쪽으로 치우쳐있는지 여부를 결정

    def lp_loss(self, pred, tgt, p=2.0):
        x = (pred - tgt).abs().pow(p)
        if self.ch_axis == -1: # 채널 축이 없는 경우 전체 텐서의 평균을 구함 
            return x.mean()
        else: # 채널 축이 있는 경우 채널 별 평균을 구한다. 
            y = _transform_to_ch_axis(x, self.ch_axis)
            return y.mean(1)

    def loss_fx(self, x, new_min, new_max):
        # should also consider channel here  // 양자화된 값 ~ 원본 값 사이의 mse 손실을 계산
        ...
        if self.ch_axis != -1:
            ...
            x_q = fake_quantize_per_tensor_affine(
                ...
                self.quant_min, self.quant_max)
        score = self.lp_loss(x_q, x, p=self.p) # x_q 양자화된 값 ~ x 원본값 사이의 loss 구하기 
        return score

    def perform_2D_search(self, x):
        # 양자화 범위를 탐색하여 최소/최대값을 찾음 
        ...
        xrange = x_max - x_min # 최소/최대 범위 계산
        best_score = torch.zeros_like(x_min) + (1e+10) # 초기 손실값을 매우 큰 값 e^10 으로 설정하여 최소화 수행할 때 갱신되도록 함 
        ...
        for i in range(1, self.num + 1):
            ...
            for zp in range(self.quant_min, self.quant_max + 1):
                ...
                # 새로운 최소/최대값이 더 좋은 손실을 내는 경우 그 값을 갱신한다. 
    ...
    def perform_1D_search(self, x): # 2d 양자화 범위탐색과 비슷하지만 1d에서는 한 축에만 집중
        if self.ch_axis != -1:
            y = _transform_to_ch_axis(x, self.ch_axis)
            x_min, x_max = torch._aminmax(y, 1)
        else:
    ...
    def forward(self, x_orig): # 입력 텐서에 대해 mse 기반으로 최소/최대값 찾기 
        if x_orig.numel() == 0:
            return x_orig
        x = x_orig.clone().detach().to(self.min_val.dtype)
...
class AvgMSEObserver(MSEObserver): # mseobserver 상속받아서 평균 최소/최대값 계산하는 observer
    def __init__(self, bit=8, symmetric=False, ch_axis=-1):
...
class MSEFastObserver(ObserverBase): 
    # golden section 탐색 방식을 사용하여 양자화 범위를 더 빠르게 찾는다. 
    # mseobserver보다 더 빠르게 최소/최대값을 탐색한다. 
